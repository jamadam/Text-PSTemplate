...
use Text::PSTemplate::DateTime qw(Strftime);
...
while (<DATA>)
{
    ...
    {
        $params = $_;
        $dt = eval "Text::PSTemplate::DateTime->new( $params, time_zone => 'UTC' )";
        next;
    }
    elsif (/^(\w+)/)
    {
        ...
        $dt = eval "Text::PSTemplate::DateTime->new( $params, time_zone => 'UTC', locale => '$locale' )";
        next;
    }
...
# context
{
    my $dt = Text::PSTemplate::DateTime->new( year => 1800,
                            month => 1,
                            day => 10,
                            time_zone => 'UTC',
                          );
...
{
    my $dt = Text::PSTemplate::DateTime->new( year => 2003,
                            hour => 0,
                            minute => 0
                          ) ;
...
{
    is( Text::PSTemplate::DateTime->new( year => 2003, month => 1, day => 1 )->strftime('%V'),
	'01', '%V is 01' );
}

{
    my $dt = Text::PSTemplate::DateTime->new( year => 2004, month => 8, day => 16,
