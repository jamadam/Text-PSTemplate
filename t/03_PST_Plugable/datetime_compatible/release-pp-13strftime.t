...
while ( defined( my $line = <DATA> ) ) {
    ...
    if ( $line =~ /^year =>/ ) {
        ...
        $dt = Text::PSTemplate::DateTime->new( %params, time_zone => 'UTC' );
        next;
    }
    elsif ( $line =~ /^(\w+)/ ) {
        $locale = $1;
        eval "use Text::PSTemplate::DateTime::Locale::$1";
        die $@ if $@;

        $dt = Text::PSTemplate::DateTime->new( %params, time_zone => 'UTC', locale => $locale );
        next;
    }
...
{
    is(
        Text::PSTemplate::DateTime->new( year => 2003, month => 1, day => 1 )->strftime('%V'),
        '01', '%V is 01'
    );
}
...

{
    ...
    my $dt = Text::PSTemplate::DateTime->from_epoch( epoch => $epoch );
...
{
    my $dt = Text::PSTemplate::DateTime->new( year => 2011 );
