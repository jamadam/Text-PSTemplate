...
my $pos        = Text::PSTemplate::DateTime::Infinite::Future->new;
my $neg        = Text::PSTemplate::DateTime::Infinite::Past->new;
my $posinf     = Text::PSTemplate::DateTime::INFINITY;
my $neginf     = Text::PSTemplate::DateTime::NEG_INFINITY;
my $nan_string = Text::PSTemplate::DateTime::NAN;
...
# infinite date math
{
    ...
    my $long_ago = Text::PSTemplate::DateTime->new( year => -100_000 );
    ...
    my $far_future = Text::PSTemplate::DateTime->new( year => 100_000 );
    ok(
        $pos > $far_future,
    ...
    ok(
        $pos == $posinf,
        "infinity (Text::PSTemplate::DateTime) == infinity (number)"
    );

    ok(
        $neg == $neginf,
        "neg infinity (Text::PSTemplate::DateTime) == neg infinity (number)"
    );
}
...

{
    ...
    is( Text::PSTemplate::DateTime->compare( $pos, $now ), 1,
        'positive infinite is greater than now' );
    is( Text::PSTemplate::DateTime->compare( $neg, $now ), -1,
        'negative infinite is less than now' );
}

{
    my $now = Text::PSTemplate::DateTime->now;
    my $pos2 = $pos + Text::PSTemplate::DateTime::Duration->new( months => 1 );

    ok(
        $pos == $pos2,
        "infinity (Text::PSTemplate::DateTime) == infinity (Text::PSTemplate::DateTime)"
    );
}

{
    my $now = Text::PSTemplate::DateTime->now;
    my $neg2 = $neg + Text::PSTemplate::DateTime::Duration->new( months => 1 );
...

{
    cmp_ok( "$pos", 'eq', $posinf,
        'stringified infinity (Text::PSTemplate::DateTime) eq infinity (number)' );
    cmp_ok( "$neg", 'eq', $neginf,
        'stringified neg infinity (Text::PSTemplate::DateTime) eq neg infinity (number)' );
}
