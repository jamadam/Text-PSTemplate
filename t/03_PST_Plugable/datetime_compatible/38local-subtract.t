...

{
    ...
    my $dur1    = $dt2->subtract_Text::PSTemplate::DateTime($dt1);
    ...
    is(
        Text::PSTemplate::DateTime->compare( $dt1->clone->add_duration($dur1), $dt2 ), 0,
        'subtract_Text::PSTemplate::DateTime is reversible from start point'
    );
    is(
        Text::PSTemplate::DateTime->compare( $dt2->clone->subtract_duration($dur1), $dt1 ), 0,
        'subtract_Text::PSTemplate::DateTime is reversible from end point'
    );
    is( $deltas1{nanoseconds}, 0, 'delta_nanoseconds is 0' );

    my $dur2    = $dt1->subtract_Text::PSTemplate::DateTime($dt2);
    ...
    is(
        Text::PSTemplate::DateTime->compare( $dt1->clone->add_duration($dur3), $dt2 ), 0,
        'delta_md is reversible from start point'
    );
    is(
        Text::PSTemplate::DateTime->compare( $dt2->clone->subtract_duration($dur3), $dt1 ), 0,
    ...
    is(
        Text::PSTemplate::DateTime->compare( $dt2->clone->subtract_duration($dur4), $dt1 ), 0,
        'delta_days is reversible from end point'
    );
}

# same as above, but now the UTC hour of the earlier Text::PSTemplate::DateTime is
...
# the docs say use UTC to guarantee reversibility
{
    ...
    my $dur = $dt2->subtract_Text::PSTemplate::DateTime($dt1);

    is(
        Text::PSTemplate::DateTime->compare( $dt1->add_duration($dur), $dt2 ), 0,
        'subtraction is reversible from start point with UTC'
    );
    is(
        Text::PSTemplate::DateTime->compare( $dt2->subtract_duration($dur), $dt2 ), 0,
        'subtraction is reversible from start point with UTC'
    );
}
...
# of subtraction done.
{
    ...
    my $dur1 = $dt2->subtract_Text::PSTemplate::DateTime_absolute($dt1);
    ...
    my $dur2 = $dt1->subtract_Text::PSTemplate::DateTime_absolute($dt2);
    ...
    is(
        Text::PSTemplate::DateTime->compare( $dt2->clone->add_duration($dur2), $dt1 ), 0,
        'subtraction is reversible'
    );
    is(
        Text::PSTemplate::DateTime->compare( $dt1->clone->subtract_duration($dur2), $dt2 ), 0,
        'subtraction is doubly reversible'
    );
}

{
    ...
    is(
        Text::PSTemplate::DateTime->compare( $dt1->clone->add_duration($dur), $dt2 ), 0,
        'subtraction is reversible'
    );
    is(
        Text::PSTemplate::DateTime->compare( $dt2->clone->subtract_duration($dur), $dt1 ), 0,
        'subtraction is doubly reversible'
    );
}

{
    ...
    is(
        Text::PSTemplate::DateTime->compare(
    ...
    is( $deltas2{seconds},     0,  'delta_seconds is 0' );
    is( $deltas2{nanoseconds}, 0,  'delta_nanoseconds is 0' );
    is( $dt2->clone->add_duration($dur2)->Text::PSTemplate::DateTime, '2003-04-05T02:58:00',
        'dt2 + dur2 != dt1' );
    is(
        Text::PSTemplate::DateTime->compare(
...
# Text::PSTemplate::DateTime objects were on the same day
{
    my $dt1 = Text::PSTemplate::DateTime->new(
    ...
    $dur = $dt1->subtract_Text::PSTemplate::DateTime($dt1);
    ok( $dur->is_zero,
...

{
    ...
    my $dur = $dt1->subtract_Text::PSTemplate::DateTime($dt1);
    ok( $dur->is_zero,
        'dst change date (with dst) - itself, duration is zero' );
}

# This tests a bug where one of the Text::PSTemplate::DateTimes is changing DST, and the
# other is not. In this case, no "adjustments" (aka hacks) are made in
# subtract_Text::PSTemplate::DateTime, and it just gives the "UTC difference".
{
    ...
    my $dur = $dt1->subtract_Text::PSTemplate::DateTime($dt2);

    is(
        $dur->delta_minutes, 720,
        'subtraction the day after a DST change in one zone, where the other Text::PSTemplate::DateTime is in a different zone'
    );
}

{
    ...
    is(
        ...
        'subtraction the day of a DST change in one zone (before the change),'
            . ' where the other Text::PSTemplate::DateTime is in a different zone'
    );
}
