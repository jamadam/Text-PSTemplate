...
use Data::Dumper;
    ...
    {
        my $parsed1 = $tpl->parse(q{<% tpl_switch($some_var1,{1 => 't/04_Plugable_Core/template/Control2.txt', 2 => ''}) %>});
        is($parsed1, 'ok');
        my $parsed2 = $tpl->parse(q{<% tpl_switch($some_var2,{1 => '', 2 => 't/04_Plugable_Core/template/Control2.txt'}) %>});
        is($parsed2, 'ok');
